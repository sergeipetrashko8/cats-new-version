    ...
    public class LabsServiceController : ApiRoutedController
    {
        ...
        public IActionResult CheckPlagiarismSubjects(string subjectId, string type, string threshold)
        {
            try
            {
                ...
                dynamic service = null; //todo # var service = new SoapWSClient();

                var result = service.checkByDirectory(new[] {PlagiarismTempPath + path}, int.Parse(threshold), 10,
                    int.Parse(type));

                var data = JsonSerializer.Deserialize<ResultPlagSubjectClu>(result);
                ...
                Directory.Delete(PlagiarismTempPath + path, true);
        ...
        public IActionResult CheckPlagiarism(string userFileId, string subjectId, bool isCp = false)
        {
            try
            {
                ...
                var result = service.checkBySingleDoc(firstFileName, new[] {PlagiarismTempPath + path}, 70, 6, 1);

                var data = JsonSerializer.Deserialize<List<ResultPlag>>(result);

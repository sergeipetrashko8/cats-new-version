...
namespace Application.Infrastructure.MessageManagement
{
    public class MessageManagementService : IMessageManagementService
    {
        ...
        public Message SaveMessage(Message message)
        {
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                ...
                if (message.Attachments != null)
                {
                    ...
                    foreach (var attach in message.Attachments) repositoriesContainer.AttachmentRepository.Save(attach);
                }
        ...
        {
            foreach (var userMsg in userMessages) SaveUserMessages(userMsg);
        }
        ...
        private static IEnumerable<User> GetRecipientsList(User currentUser)
        {
            ...
            if (currentUser?.Membership?.Roles == null || !currentUser.Membership.Roles.Any()) return recipientsList;

            var role = currentUser.Membership.Roles.Select(r => r.Role.RoleName).ToArray().First();

            using var repositoriesContainer = new LmPlatformRepositoriesContainer();

            recipientsList = role switch
            {
                Constants.Roles.Student => repositoriesContainer.UsersRepository
                    .GetAll(new Query<User>(u => u.Lecturer != null).Include(u => u.Lecturer))
                    .ToList(),
                Constants.Roles.Lector => repositoriesContainer.UsersRepository
                    .GetAll(new Query<User>(u => u.Lecturer != null || u.Student != null).Include(u => u.Student)
                        .Include(u => u.Lecturer))
                    .ToList(),
                Constants.Roles.Admin => repositoriesContainer.UsersRepository
                    .GetAll(new Query<User>(u => u.Id != currentUser.Id).Include(u => u.Student)
                        .Include(u => u.Lecturer))
                    .ToList(),
                _ => recipientsList
            };

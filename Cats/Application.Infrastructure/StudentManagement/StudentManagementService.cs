...
namespace Application.Infrastructure.StudentManagement
{
    public class StudentManagementService : IStudentManagementService
    {
        ...
        public Student GetStudent(int userId)
        {
            ...
            return repositoriesContainer.StudentsRepository.GetBy(new Query<Student>(e => e.Id == userId)
                .Include(e => e.Group).Include(e => e.User));
        }
        ...
        public IEnumerable<Student> GetStudents()
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.StudentsRepository
                .GetAll(new Query<Student>().Include(e => e.Group).Include(e => e.User)).ToList();
        }

        public IPageableList<Student> GetStudentsPageable(string searchString = null, IPageInfo pageInfo = null,
        ...
        public int CountUnconfirmedStudents(int lecturerId)
        {
            ...
            var subjects = repositoriesContainer.RepositoryFor<SubjectLecturer>().GetAll(
                new Query<SubjectLecturer>(e => e.LecturerId == lecturerId).Include(e =>
                    e.Subject.SubjectGroups.Select(x => x.Group)));

            var groupIds = new List<int>();

            foreach (var subject in subjects) groupIds.AddRange(subject.Subject.SubjectGroups.Select(e => e.GroupId));
                        ...
                        e => e.StudentId == student.Id))
                {
                    if (subGroup.SubGroups != null && subGroup.SubGroups.Any())
                    {
                        var modelFirstSubGroup =
                            subGroup.SubGroups.FirstOrDefault(e => e.Name == "first").SubjectStudents ??
                            new List<SubjectStudent>();

                        var first = modelFirstSubGroup != null
                            ? modelFirstSubGroup.Select(e => e.StudentId).ToList()
                            : new List<int>();

                        first.Add(student.Id);

                        var modelSecondSubGroup =
                            subGroup.SubGroups.FirstOrDefault(e => e.Name == "second").SubjectStudents ??
                            new List<SubjectStudent>();

                        var modelSecondThirdGroup =
                            subGroup.SubGroups.FirstOrDefault(e => e.Name == "third").SubjectStudents ??
                            new List<SubjectStudent>();

                        repositoriesContainer.SubGroupRepository.SaveStudents(subject.Id, subGroup.Id, first,
                            modelSecondSubGroup.Select(e => e.StudentId).ToList(),
                            modelSecondThirdGroup.Select(e => e.StudentId).ToList());
                    }
                    else
                    {
                        var students = GetGroupStudents(student.GroupId)
                            .Where(e => e.Confirmed == null || e.Confirmed.Value);

                        repositoriesContainer.SubGroupRepository.CreateSubGroup(subject.Id, subGroup.Id,

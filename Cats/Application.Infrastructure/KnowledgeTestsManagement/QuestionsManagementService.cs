...
namespace Application.Infrastructure.KnowledgeTestsManagement
{
    public class QuestionsManagementService : IQuestionsManagementService
    {
        public Question GetQuestion(int id)
        {
            ...
            return repositoriesContainer.QuestionsRepository.GetBy(
                new Query<Question>(question => question.Id == id)
                    .Include(question => question.Answers)
                    .Include(q => q.ConceptQuestions.Select(cq => cq.Concept)));
        }

        public void ChangeQuestionNumber(int questionId, int number)
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            var question = repositoriesContainer.QuestionsRepository
                .GetAll(new Query<Question>(x => x.Id == questionId)).FirstOrDefault();
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            var test = repositoriesContainer.TestsRepository.GetAll(new Query<Test>(x => x.Id == testId))
                .FirstOrDefault();
        ...
        public Question SaveQuestion(Question question)
        {
            ...
            foreach (var answer in question.Answers) answer.QuestionId = question.Id;
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            var questionToDelete = repositoriesContainer.QuestionsRepository.GetBy(
                new Query<Question>(question => question.Id == id));
        ...
        public IEnumerable<Question> GetQuestionsForTest(int testId, string searchString = null)
        {
            ...
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                ...
                if (testId != 0) query.AddFilterClause(question => question.TestId == testId);

                if (searchString != null) query.AddFilterClause(question => question.Title.Contains(searchString));
        ...
        private void ValidateQuestion(Question question)
        {
            ...
            if (question.ComlexityLevel <= 0) throw new InvalidDataException("Сложность теста должна быть боьше нуля");
        ...
        {
            if (question.Answers.Count < 3) throw new InvalidDataException("Вопрос должен иметь хотя бы 3 варианта");

            var correctAnswersCount = question.Answers.Count(answer => answer.СorrectnessIndicator > 0);
        ...
        {
            if (question.Answers.Count < 2) throw new InvalidDataException("Вопрос должен иметь хотя бы 2 варианта");

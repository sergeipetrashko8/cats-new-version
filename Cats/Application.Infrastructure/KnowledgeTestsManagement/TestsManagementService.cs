...
namespace Application.Infrastructure.KnowledgeTestsManagement
{
    public class TestsManagementService : ITestsManagementService
    {
        ...
        public void DeleteTest(int id)
        {
            ...
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            var testToDelete = repositoriesContainer.TestsRepository.GetBy(
        ...
        public void UnlockTest(int[] studentIds, int testId, bool unlock)
        {
            ...
            var savedTestUnlocks = repositoriesContainer.TestUnlocksRepository.GetAll(new
                        Query<TestUnlock>()
                    .AddFilterClause(testUnlock =>
                        studentIds.Contains(testUnlock.StudentId) && testUnlock.TestId == testId))
                .ToList();

            repositoriesContainer.TestUnlocksRepository.Delete(savedTestUnlocks);
            if (unlock)
            {
                var testUnlocks = studentIds.Select(studentId => new TestUnlock
                {
                    StudentId = studentId,
                    TestId = testId
                });
        ...
        public void UnlockTestForStudent(int testId, int studentId, bool unlocked)
        {
            ...
            var savedTestUnlock = repositoriesContainer.TestUnlocksRepository.GetAll(
                    new Query<TestUnlock>().AddFilterClause(
                        testUnlock => testUnlock.StudentId == studentId && testUnlock.TestId == testId))
                .SingleOrDefault();
            ...
            {
                if (savedTestUnlock != null) repositoriesContainer.TestUnlocksRepository.Delete(savedTestUnlock);
            }
        ...
        public Test GetTest(int id, bool includeQuestions = false)
        {
            ...
            if (includeQuestions) query.Include(t => t.Questions);
        ...
        private void ValidateTest(Test test)
        {
            ...
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            if (repositoriesContainer.TestsRepository.GetAll(new Query<Test>(t =>
                t.Id != test.Id && t.Title == test.Title && t.SubjectId == test.SubjectId)).Any())
        ...
        public IEnumerable<TestUnlockInfo> GetTestUnlocksForTest(int groupId, int testId, string searchString = null)
        {
            ...
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                ...
                testUnlocksResults = GetTestUnlocks(studentResults.Select(student => student.Id).ToArray(), testId,
                    repositoriesContainer).ToList();
            }
            ...
            foreach (var testUnlockInfo in results) testUnlockInfo.Number = counter++;
        ...
        private static IEnumerable<TestUnlock> GetTestUnlocks(int[] studentIds, int testId,
        ...
        private IEnumerable<Student> GetStudents(int groupId, string searchString,
            LmPlatformRepositoriesContainer repositoriesContainer)
        {
            ...
            var students = repositoriesContainer.StudentsRepository.GetAll(studentsQuery);

...
namespace Application.Infrastructure.BugManagement
{
    public class BugManagementService : IBugManagementService
    {
        ...
        public List<Bug> GetUserBugs(int userId, int pageSize, int pageNumber, string sortingPropertyName, bool desc,
            string searchString)
        {
            searchString = ValidateSearchString(searchString);
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.BugsRepository.GetUserBugs(userId, pageSize, (pageNumber - 1) * pageSize,
                searchString, sortingPropertyName, desc);
        }
        ...
        public List<Bug> GetProjectBugs(int projectId, int pageSize, int pageNumber, string sortingPropertyName,
            bool desc, string searchString)
        {
            searchString = ValidateSearchString(searchString);
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.BugsRepository.GetProjectBugs(projectId, pageSize,
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.BugLogsRepository
                .GetAll(new Query<BugLog>(e => e.BugId == bugId).Include(e => e.User)).ToList();
        }
        ...
        public void ClearBugLogs(int bugId)
        {
            ...
            foreach (var bugLog in bugLogs) repositoriesContainer.BugLogsRepository.DeleteBugLog(bugLog);

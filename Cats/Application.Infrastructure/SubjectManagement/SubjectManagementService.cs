...
namespace Application.Infrastructure.SubjectManagement
{
    public class SubjectManagementService : ISubjectManagementService
    {
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.SubjectRepository.GetSubjects(groupId).Where(e => !e.IsArchive).ToList();
        }
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.LabsRepository.GetAll(new Query<Labs>(e => e.SubjectId == subjectId))
                .ToList();
        }

        public IPageableList<Subject> GetSubjectsLecturer(int lecturerId, string searchString = null,
            IPageInfo pageInfo = null, IEnumerable<ISortCriteria> sortCriterias = null)
        {
            var query = new PageableQuery<Subject>(pageInfo,
                e => e.SubjectLecturers.Any(x => x.LecturerId == lecturerId && x.Owner == null && !e.IsArchive));
        ...
        public SubjectNews GetNews(int id, int subjecttId)
        {
            ...
            return repositoriesContainer.SubjectRepository
                .GetBy(new Query<Subject>(e => e.Id == subjecttId).Include(e => e.SubjectNewses)).SubjectNewses
        ...
        public List<SubjectNews> GetNewsByGroup(int id)
        {
            ...
            var subjects =
                repositoriesContainer.RepositoryFor<SubjectGroup>()
                    .GetAll(new Query<SubjectGroup>(e => e.GroupId == id && e.IsActiveOnCurrentGroup)).Select(
                        e => e.SubjectId).ToList();

            var subjectsData =
                repositoriesContainer.RepositoryFor<SubjectGroup>()
                    .GetAll(new Query<SubjectGroup>(e => e.GroupId == id && e.IsActiveOnCurrentGroup).Include(e =>
                        e.Subject)).ToList();
            ...
            var subjectGroup =
                repositoriesContainer.SubjectRepository.GetBy(
                    new Query<Subject>(e => e.Id == subjectId && e.SubjectGroups.Any(x => x.GroupId == groupId))
                        .Include(e => e.SubjectGroups.Select(x =>
                            x.SubGroups.Select(c =>
                                c.SubjectStudents.Select(t => t.Student.ScheduleProtectionLabMarks))))
                        .Include(e => e.SubjectGroups.Select(x => x.SubGroups.Select(c =>
                            c.SubjectStudents.Select(t => t.Student.ScheduleProtectionPracticalMarks))))
                        .Include(e => e.SubjectGroups.Select(x =>
                            x.SubGroups.Select(c => c.SubjectStudents.Select(t => t.Student.StudentLabMarks))))
                        .Include(e => e.SubjectGroups.Select(x =>
                            x.SubGroups.Select(c => c.SubjectStudents.Select(t => t.Student.User))))
                        .Include(e => e.SubjectGroups.Select(x =>
                            x.SubGroups.Select(c =>
                                c.SubjectStudents.Select(t => t.Student.StudentPracticalMarks))))
                        .Include(
                            e => e.SubjectGroups.Select(x => x.SubGroups.Select(c => c.ScheduleProtectionLabs))));
            ...
            IList<int> thirdInts)
        {
            ...
            var subject = repositoriesContainer.SubjectRepository.GetBy(
                new Query<Subject>(e => e.Id == subjectId && e.SubjectGroups.Any(x => x.GroupId == groupId))
                    .Include(e => e.SubjectGroups.Select(x => x.SubGroups)));
            var firstOrDefault = subject.SubjectGroups.FirstOrDefault(e => e.GroupId == groupId);
            if (firstOrDefault.SubGroups.Any())
                repositoriesContainer.SubGroupRepository.SaveStudents(subjectId, firstOrDefault.Id, firstInts,
                    secoInts, thirdInts);
            else
                repositoriesContainer.SubGroupRepository.CreateSubGroup(subjectId, firstOrDefault.Id, firstInts,
                    secoInts, thirdInts);
        }
        ...
        public Lectures GetLectures(int id)
        {
            ...
            return repositoriesContainer.LecturesRepository.GetBy(
                new Query<Lectures>(e => e.Id == id).Include(e => e.Subject));
        }
        ...
        public UserLabFiles SaveUserLabFiles(UserLabFiles userLabFiles, IList<Attachment> attachments)
        {
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                if (!string.IsNullOrEmpty(userLabFiles.Attachments))
                {
                    var deleteFiles =
                        repositoriesContainer.AttachmentRepository.GetAll(
                                new Query<Attachment>(e => e.PathName == userLabFiles.Attachments)).ToList()
                            .Where(e => attachments.All(x => x.Id != e.Id)).ToList();
        ...
        public Labs GetLabs(int id)
        {
            ...
            return repositoriesContainer.LabsRepository.GetBy(
                new Query<Labs>(e => e.Id == id).Include(e => e.Subject));
        }
        ...
        public Practical GetPractical(int id)
        {
            ...
            return repositoriesContainer.PracticalRepository.GetBy(
                new Query<Practical>(e => e.Id == id).Include(e => e.Subject));
        }
        ...
        public SubGroup GetSubGroup(int subGroupId)
        {
            ...
            return
                repositoriesContainer.RepositoryFor<SubGroup>().GetBy(new Query<SubGroup>(e => e.Id == subGroupId)
                    ...
                    .Include(e => e.SubjectGroup.Group));
        }
        ...
        public List<string> GetLecturesAttachments(int subjectId)
        {
            ...
            model.AddRange(
                repositoriesContainer.LecturesRepository
                    .GetAll(new Query<Lectures>(e => e.SubjectId == subjectId).Include(e => e.Attachments))
                    .Select(e => e.Attachments).ToList());
            return model;
        }
        ...
        public List<string> GetLabsAttachments(int subjectId)
        {
            ...
            var model = new List<string>();
            model.AddRange(
                repositoriesContainer.LabsRepository
                    .GetAll(new Query<Labs>(e => e.SubjectId == subjectId).Include(e => e.Attachments))
                    .Select(e => e.Attachments).ToList());
            return model;
        }
        ...
        public List<string> GetPracticalsAttachments(int subjectId)
        {
            ...
            var model = new List<string>();
            model.AddRange(
                repositoriesContainer.PracticalRepository
                    .GetAll(new Query<Practical>(e => e.SubjectId == subjectId).Include(e => e.Attachments))
                    .Select(e => e.Attachments).ToList());
            return model;
        }
        ...
        public UserLabFiles GetUserLabFile(string path)
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.RepositoryFor<UserLabFiles>()
                .GetBy(new Query<UserLabFiles>(e => e.Attachments == path));
        }
                ...
                foreach (var subject in subjects)
                {
                    ...
                    foreach (var scheduleProtectionLabse in subGroup.ScheduleProtectionLabs)
                        model.Add(new ProfileCalendarModel
                        {
                            ...
                            Title = $"{name} -  Лаб.работа (Гр. {group.Group.Name})",
        ...
        public List<ProfileCalendarModel> GetGroupsLabEvents(int groupId, int userId)
        {
            ...
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                ...
                foreach (var subject in subjects)
                {
                    ...
                    foreach (var group in subject.SubjectGroups)
                    foreach (var subGroup in group.SubGroups.Where(e =>
                        e.SubjectStudents.Any(x => x.StudentId == userId)))
                    foreach (var scheduleProtectionLabse in subGroup.ScheduleProtectionLabs)
                        model.Add(new ProfileCalendarModel
                        {
                            Start = scheduleProtectionLabse.Date.ToString("yyyy-MM-dd"),
                            Title = $"{name} -  Лаб.работа",
                            SubjectId = subject.Id,
                            Color = subject.Color
                        });
                }
        ...
        public List<ProfileCalendarModel> GetLecturesEvents(int groupId, int userId)
        {
            ...
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                ...
                foreach (var subject in subjects)
                {
                    ...
                    model.AddRange(subject.LecturesScheduleVisitings
                        .Select(lecturesScheduleVisiting => new ProfileCalendarModel
                        {
                            Start = lecturesScheduleVisiting.Date.ToString("yyyy-MM-dd"),
                            Title = $"{name} -  Лекция", Color = subject.Color,
                            SubjectId = subject.Id
                        }));
                }
            }
        ...
        public List<Subject> GetSubjectsByLector(int userId)
        {
            ...
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                model = repositoriesContainer.SubjectRepository.GetSubjects(lecturerId: userId).Where(e => !e.IsArchive)
                    .ToList();
            }
        ...
        public List<Subject> GetSubjectsByLectorOwner(int userId)
        {
            ...
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            model = repositoriesContainer.RepositoryFor<SubjectLecturer>().GetAll(
                    new Query<SubjectLecturer>(e => e.LecturerId == userId && e.Owner == null)
                        ...
                        .Include(e =>
                            e.Subject.SubjectGroups.Select(x => x.SubGroups.Select(t => t.ScheduleProtectionLabs))))
                .Select(e => e.Subject).Where(e => !e.IsArchive).ToList();
            return model;
        }
        ...
        public decimal GetSubjectCompleting(int subjectId, string user, Student student)
        {
            ...
            if (user == "S")
            {
                ...
                var marks = repositoriesContainer.StudentsRepository
                    .GetBy(new Query<Student>(e => e.Id == student.Id).Include(e => e.StudentLabMarks))
                    .StudentLabMarks.Where(e => labs.Any(x => x.Id == e.LabId)).ToList()
                    .Count; //student.StudentLabMarks.Count;
            ...
            else
            {
                ...
                if (isDate)
                {
                    ...
                    var countDone = dates.Count(dateTime => nowDate > dateTime);

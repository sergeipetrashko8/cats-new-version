...
namespace Application.Infrastructure.ProjectManagement
{
    public class ProjectManagementService : IProjectManagementService
    {
        ...
        public List<Project> GetUserProjects(int userId, int pageSize, int pageNumber, string sortingPropertyName,
            bool desc, string searchString)
        {
            if (searchString != null && searchString.Length < 3)
                searchString = null;
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.ProjectsRepository.GetUserProjects(userId, pageSize,
                (pageNumber - 1) * pageSize, searchString, sortingPropertyName, desc);
        }
        ...
        public List<Project> GetUserProjectParticipations(int userId, int pageSize, int pageNumber,
            string sortingPropertyName, bool desc, string searchString)
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.ProjectsRepository.GetUserProjectParticipations(userId, pageSize,
                (pageNumber - 1) * pageSize, searchString, sortingPropertyName, desc);
        }
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.ProjectsRepository.GetStudentsGroupParticipations(groupId, pageSize,
        ...
        public Project GetProject(int projectId, bool includeBugs = false, bool includeUsers = false)
        {
            ...
            if (includeBugs) query.Include(e => e.Bugs);

            if (includeUsers) query.Include(e => e.ProjectUsers);
        ...
        public List<ProjectUser> GetProjectUsers(int projectId)
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return
                repositoriesContainer.ProjectUsersRepository
                    .GetAll(new Query<ProjectUser>(e => e.ProjectId == projectId).Include(e => e.User))
                    .ToList();
        }
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.ProjectUsersRepository
                .GetAll(new Query<ProjectUser>(e => e.UserId == userId).Include(e => e.Project).Include(e => e.User)
                    .Include(e => e.ProjectRole)).ToList();
        }

        public ProjectUser GetProjectUser(int projectUserId)
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.ProjectUsersRepository.GetBy(
                new Query<ProjectUser>(e => e.Id == projectUserId).Include(e => e.Project).Include(e => e.User)
                    .Include(e => e.ProjectRole));
        }
        ...
        public IPageableList<ProjectUser> GetProjectUsers(string searchString = null, IPageInfo pageInfo = null,
            IEnumerable<ISortCriteria> sortCriterias = null)
        {
            ...
            if (!string.IsNullOrEmpty(searchString))
                query.AddFilterClause(
                    e => e.User.FullName.ToLower().StartsWith(searchString) ||
                         e.User.FullName.ToLower().Contains(searchString));
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            return repositoriesContainer.ProjectsRepository.GetAll(new Query<Project>().Include(e => e.Creator))
                .ToList();
        }

        public IPageableList<Project> GetProjects(string searchString = null, IPageInfo pageInfo = null,
            IEnumerable<ISortCriteria> sortCriterias = null)
        {
        ...
        public ProjectMatrixRequirement CreateMatrixRequirement(ProjectMatrixRequirement requirement)
        {
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                // todo unused repositoriesContainer.ProjectMatrixRequirementsRepository.Save(requirement);
                repositoriesContainer.ApplyChanges();
            }
        ...
        public void ClearProject(int projectId)
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            var project =
                repositoriesContainer.ProjectsRepository.GetBy(new Query<Project>(e => e.Id == projectId));
            var projectUserList =
            ...
            foreach (var bug in bugList) repositoriesContainer.BugsRepository.DeleteBug(bug);
        }
        ...
        public void DeleteProjectUser(int projectUserId)
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            var projectUser =
                repositoriesContainer.ProjectUsersRepository.GetBy(
                    new Query<ProjectUser>().AddFilterClause(u => u.Id == projectUserId));
            repositoriesContainer.ProjectUsersRepository.DeleteProjectUser(projectUser);
            repositoriesContainer.ApplyChanges();
        }
        ...
        {
            var project = GetProject(projectId, true, true);
            ...
            foreach (var projectUser in projectUsers) DeleteProjectUser(projectUser.Id);
        ...
        public Attachment SaveAttachment(int projectId, Attachment attachment)
        {
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                ...
                if (string.IsNullOrEmpty(project.Attachments)) project.Attachments = GetGuidFileName();
        ...
        public List<Attachment> GetAttachments(int projectId)
        {
            ...
            if (string.IsNullOrEmpty(project.Attachments)) return new List<Attachment>();
        ...
        public Attachment GetAttachment(int projectId, string fileName)
        {
            ...
            if (string.IsNullOrEmpty(project.Attachments)) return null;
        ...
        public bool DeleteAttachment(int projectId, string fileName)
        {
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                ...
                var attachment = repositoriesContainer.AttachmentRepository.GetBy(
                    new Query<Attachment>(e => e.PathName == project.Attachments && e.FileName == fileName));

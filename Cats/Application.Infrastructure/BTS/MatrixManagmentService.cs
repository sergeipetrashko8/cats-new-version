...
namespace Application.Infrastructure.BTS
{
    ...
    //    private IProjectManagementService ProjectManagementService => _projectManagementService.Value;
    ...
    //            if (match.Success)
    //            {
    //                if (!filteredSheets.ContainsKey(sheet.Key)) filteredSheets[sheet.Key] = new List<string>();
    ...
    //        foreach (Match match in regex.Matches(text)) result[match.Groups[1].Value] = match.Groups[3].Value;

    ...
    public class LecturerManagementService : ILecturerManagementService
    {
        ...
        public List<List<string>> GetLecturesScheduleMarks(int subjectId, int groupId)
        {
            ...
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                var group = repositoriesContainer.GroupsRepository.GetBy(
                    new Query<Group>(e => e.Id == groupId).Include(e =>
                        e.Students.Select(x => x.LecturesVisitMarks.Select(t => t.LecturesScheduleVisiting))));
                ...
                foreach (var student in group.Students)
                {
                    ...
                    row.AddRange(student.LecturesVisitMarks.OrderBy(e => e.LecturesScheduleVisiting.Date)
                        .Select(lecturesVisitMark => lecturesVisitMark.Mark));
        ...
        public List<string> GetLecturesScheduleVisitings(int subjectId)
        {
            ...
            using (var repositoriesContainer = new LmPlatformRepositoriesContainer())
            {
                ...
                data.AddRange(subject.LecturesScheduleVisitings.OrderBy(e => e.Date).Select(lecturesScheduleVisiting =>
                    lecturesScheduleVisiting.Date.ToString("dd/MM/yyyy")));
            }
        ...
        {
            using var repositoriesContainer = new LmPlatformRepositoriesContainer();
            var query = repositoriesContainer.LecturerRepository.GetAll(new Query<Lecturer>()
                .Include(e => e.SubjectLecturers).Include(e => e.User));
            if (orderBy != null) query = query.OrderBy(orderBy);
            return query.ToList();
        }

        public IPageableList<Lecturer> GetLecturersPageable(string searchString = null, IPageInfo pageInfo = null,
            ...
            var relation =
                repositoriesContainer.RepositoryFor<SubjectLecturer>().GetBy(
                    new Query<SubjectLecturer>(e =>
                        e.Owner == owner && e.LecturerId == lectorId && e.SubjectId == subjectId));

...
namespace Application.Infrastructure.DPManagement
{
    public class DpManagementService : IDpManagementService
    {
        ...
        public PagedList<DiplomProjectData> GetProjects(int userId, GetPagedListParams parms)
        {
            ...
            IQueryable<DiplomProject> query = Context.DiplomProjects.AsNoTracking()
            ...
            {
                var diplomaProjects = from dp in query
                    ...
                    select new DiplomProjectData
                    {
                        ...
                        Theme = dp.Theme,
                        Lecturer = dp.Lecturer != null
                            ? dp.Lecturer.LastName + " " + dp.Lecturer.FirstName + " " + dp.Lecturer.MiddleName
                            : null,
                        Student = adp.Student != null
                            ? adp.Student.LastName + " " + adp.Student.FirstName + " " + adp.Student.MiddleName
                            : null,
                ...
                return diplomaProjects.ApplyPaging(parms);
            }
            else 
            {
                var diplomaProjects = from dp in query
        ...
        public void SaveProject(DiplomProjectData projectData)
        {
            ...
            var newGroups = projectData.SelectedGroupsIds.Select(x => new DiplomProjectGroup
                {GroupId = x, DiplomProjectId = project.DiplomProjectId}).ToList();
            ...
            foreach (var projectGroup in groupsToAdd) project.DiplomProjectGroups.Add(projectGroup);

            foreach (var projectGroup in groupsToDelete) Context.DiplomProjectGroups.Remove(projectGroup);
        ...
        public void AssignProject(int userId, int projectId, int studentId)
        {
            ...
            if (isLecturer && assignment?.ApproveDate != null
        ...
        public List<NewsData> GetNewses(int lecturerId)
        {
            ...
            if (user)
            {
                var newses = Context.DiplomProjectNews.Where(x => x.LecturerId == lecturerId)
            ...
            {
                var newses = Context.DiplomProjectNews.OrderByDescending(x => x.EditDate);
                foreach (DiplomProjectNews cp in newses)
                {
                    var n = new NewsData();
                    n.Id = cp.Id;
        ...
        {
            var models = Context.DiplomProjectNews.Where(e => e.LecturerId == subjectId);
        ...
        public void SaveTaskSheetTemplate(DiplomProjectTaskSheetTemplate template)
        {
            var existingTemplate =
                Context.DiplomProjectTaskSheetTemplates.FirstOrDefault(x => x.Name.Trim() == template.Name.Trim())
        ...
        public DiplomProjectNews SaveNews(DiplomProjectNews news, IList<Attachment> attachments, int userId)
        {
            ...
            var cpEditNews =
                Context.DiplomProjectNews.FirstOrDefault(x => x.Id == news.Id && x.LecturerId == news.LecturerId);
            if (cpEditNews != null)
            {
                //CourseProjectNews cpnews = new CourseProjectNews(); todo unused
            ...
            {
                Context.DiplomProjectNews.Add(news);
            }
        ...
        {
            var cpnews = Context.DiplomProjectNews.Single(x => x.Id == news.Id && x.LecturerId == news.LecturerId);
            Context.DiplomProjectNews.Remove(cpnews);
            Context.SaveChanges();
        }

        public DiplomProjectNews GetNews(int id)
        {
            return Context.DiplomProjectNews.Single(x => x.Id == id);
        }
    }
}
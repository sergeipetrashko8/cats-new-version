    ...
    public class CorrelationService : ICorrelationService
    {
        ...
        private List<Correlation> GetGroupsCorrelation(int? userId)
        {
            ...
            if (userId.HasValue)
            {
                ...
                if (isSecretary) groups = groups.Where(x => !x.SecretaryId.HasValue || x.SecretaryId == userId);
            }
        ...
        {
            if (!id.HasValue) throw new ApplicationException("userId cant be null!");
            ...
            return AuthorizationHelper.IsLecturer(Context, id.Value)
                ? Context.Lecturers
                    ...
                    {
                        Id = x.secretary.Id,
                        Name = $"{x.secretary.FullName} ({string.Join(", ", x.groups.Select(g => g.Name).Distinct())})"
                    }).ToList()
                : AuthorizationHelper.IsStudent(Context, id.Value)
                    ? Context.Students.Where(x => x.Id == id).Select(x => x.Group.Secretary).Where(x => x != null)
                        .ToList()
        ...
        {
            var projects = lecturerId.HasValue
                ? Context.DiplomProjects.Where(x => x.LecturerId == lecturerId)
                : Context.DiplomProjects;
        ...
        private List<Correlation> GetDiplomLecturerCorrelation(int? id)
        {
            return Context.Lecturers
                .Where(x => x.IsLecturerHasGraduateStudents &&
                            x.DiplomProjects.Any(dp => dp.AssignedDiplomProjects.Any()))
                .ToList()
                .Select(x => new Correlation
                {
                    Id = x.Id,

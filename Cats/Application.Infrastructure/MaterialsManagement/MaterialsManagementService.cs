    ...
    public class MaterialsManagementService : IMaterialsManagementService
    {
        public List<Folders> GetFolders(int PID, int subjectId)
        {
            ...
            var subject = repositoriesContainer.SubjectRepository.GetBy(
                new Query<Subject>(e => e.Id == subjectId).Include(e => e.SubjectModules.Select(x => x.Module)));

            var submod = subject.SubjectModules.FirstOrDefault(e => e.ModuleId == 9);
        ...
        public int GetPidById(int PID)
        {
            ...
            if (PID != 0) id = repositoriesContainer.FoldersRepository.GetPidById(PID);

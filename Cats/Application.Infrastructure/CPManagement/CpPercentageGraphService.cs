...
namespace Application.Infrastructure.CPManagement
{
    public class CpPercentageGraphService : ICpPercentageGraphService
    {
        ...
        {
            var groupId = 0;
            if (parms.Filters.ContainsKey("groupId")) int.TryParse(parms.Filters["groupId"], out groupId);

            var subjectId = 0;
            if (parms.Filters.ContainsKey("subjectId")) int.TryParse(parms.Filters["subjectId"], out subjectId);
        ...
        public List<CourseProjectConsultationDateData> GetConsultationDatesForUser(int userId, int subjectId)
        {
            if (AuthorizationHelper.IsStudent(Context, userId))
            {
                ...
                if (student.AssignedCourseProjects.Count == 0) return new List<CourseProjectConsultationDateData>();
        ...
        public void SavePercentage(int userId, PercentageGraphData percentageData)
        {
            ...
            if (percentageData.Id.HasValue)
            {
                ...
                if (Context.CoursePercentagesGraphs.Any(x =>
                    x.SubjectId == percentageData.SubjectId && x.Name == percentageData.Name &&
                    x.Id != percentageData.Id)) throw new ApplicationException("Этап с таким названием уже есть!");
            }
            else
            {
                if (Context.CoursePercentagesGraphs.Any(x =>
        ...
        /// <returns></returns>
        private IQueryable<PercentageGraphData> GetPercentageGraphDataForLecturerQuery(int lecturer, int secretaryId,
            int subjectId)
        {
            var p = Context.CoursePercentagesGraphs.Where(x => x.LecturerId == secretaryId)
                .Where(x => x.SubjectId == subjectId).Select(ToPercentageDataPlain);

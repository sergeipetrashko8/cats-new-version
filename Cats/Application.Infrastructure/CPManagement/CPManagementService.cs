...
namespace Application.Infrastructure.CPManagement
{
    public class CPManagementService : ICPManagementService
    {
        ...
        public PagedList<CourseProjectData> GetProjects(int userId, GetPagedListParams parms)
        {
            ...
            var user = Context.Users.Include(x => x.Student).Include(x => x.Lecturer)
                .SingleOrDefault(x => x.Id == userId);

            if (user?.Lecturer != null)
                query = (IIncludableQueryable<CourseProject, Subject>) query.Where(x => x.LecturerId == userId)
                    .Where(x => x.SubjectId == subjectId);

            if (user?.Student != null)
                query = (IIncludableQueryable<CourseProject, Subject>) query.Where(x =>
                        x.CourseProjectGroups.Any(dpg => dpg.GroupId == user.Student.GroupId))
                    .Where(x => x.SubjectId == subjectId);
            ...
            if (searchString.Length > 0)
            {
                var courseProjects = from cp in query
                    ...
                    select new CourseProjectData
                    {
                        ...
                        Theme = cp.Theme,
                        Lecturer = cp.Lecturer != null
                            ? cp.Lecturer.LastName + " " + cp.Lecturer.FirstName + " " + cp.Lecturer.MiddleName
                            : null,
                        Student = acp.Student != null
                            ? acp.Student.LastName + " " + acp.Student.FirstName + " " + acp.Student.MiddleName
                            : null,
        ...
        {
            if (!projectData.LecturerId.HasValue) throw new ApplicationException("LecturerId cant be empty!");
            ...
            var newGroups = projectData.SelectedGroupsIds.Select(x => new CourseProjectGroup
                {GroupId = x, CourseProjectId = project.CourseProjectId}).ToList();
            ...
            foreach (var projectGroup in groupsToAdd) project.CourseProjectGroups.Add(projectGroup);

            foreach (var projectGroup in groupsToDelete) Context.CourseProjectGroups.Remove(projectGroup);
        ...
        public void AssignProject(int userId, int projectId, int studentId)
        {
            ...
            if (cp.Subject.IsNeededCopyToBts) CreateBtsProject(cp, studentId);
        }
        ...
        public PagedList<StudentData> GetGraduateStudentsForUser(int userId, int subjectId, GetPagedListParams parms,
            bool getBySecretaryForStudent = true)
        {
            var secretaryId = 0;
            if (parms.Filters.ContainsKey("secretaryId")) int.TryParse(parms.Filters["secretaryId"], out secretaryId);
            ...
            if (isStudent)
            {
                if (getBySecretaryForStudent)
                    ...
                    userId = Context.Users.Where(x => x.Id == userId)
                                 .Select(x =>
                                     x.Student.AssignedCourseProjects.FirstOrDefault().CourseProject.LecturerId)
                                 .Single() ?? 0;
            }
            ...
            var query = Context.Students
                .Where(x => isLecturerSecretary || isStudent && getBySecretaryForStudent ||
                            x.AssignedCourseProjects.Any(asd =>
                                asd.CourseProject.LecturerId == userId && asd.CourseProject.SubjectId == subjectId))
            ...
            GetPagedListParams parms, bool getBySecretaryForStudent = true)
        {
            ...
            if (parms.Filters.ContainsKey("searchString")) searchString = parms.Filters["searchString"];
        ...
        {
            var projects = Context.CourseProjects.Where(e => e.SubjectId == subjectId)
                .Include(x => x.CourseProjectGroups);
            foreach (var project in projects)
            foreach (var groupId in groupIds)
            {
                var projectGroup = new CourseProjectGroup
                {
        ...
        public void DisableNews(int subjectId, bool disable)
        {
            ...
            foreach (var cpNews in models) cpNews.Disabled = disable;
        ...
        public void DeletePercenageAndVisitStatsForUser(int id)
        {
            ...
            foreach (var cp in cpPR) Context.CoursePercentagesResults.Remove(cp);

            var cpVS = Context.CourseProjectConsultationMarks.Where(e => e.StudentId == id);
            foreach (var cp in cpVS) Context.CourseProjectConsultationMarks.Remove(cp);
            Context.SaveChanges();
        }

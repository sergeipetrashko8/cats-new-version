...
namespace LMP.Data.Repositories
{
    public class FoldersRepository : RepositoryBase<LmPlatformModelsContext, Folders>, IFoldersRepository
    {
        ...
        public List<Folders> GetFoldersByPIDandSubId(int pid, int idsubjectmodule)
        {
            ...
            var folders = context.Set<Folders>().Where(e => e.Pid == pid && e.SubjectModule.Id == subjectmodule.Id)
                .ToList();
            return folders;
        }
        ...
        public List<Folders> GetFoldersByPID(int pid)
        {
            ...
            var folders = context.Set<Folders>()
                .Where(e => e.Pid == pid && e.SubjectModule.Subject.IsArchive == false).ToList();
            return folders;
        }

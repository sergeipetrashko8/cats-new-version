...
namespace LMP.Data.Repositories
{
    public class ProjectsRepository : RepositoryBase<LmPlatformModelsContext, Project>, IProjectsRepository
    {
        ...
        public List<Project> GetUserProjects(int userId, int limit, int offset, string searchString,
            string sortingPropertyName, bool desc)
        {
        ...
        public List<Project> GetUserProjectParticipations(int userId, int limit, int offset, string searchString,
            string sortingPropertyName, bool desc = false)
        {
            using var context = new LmPlatformModelsContext();
        ...
        private IQueryable<Project> GetUserProjectsQuery(LmPlatformModelsContext context, int userId,
            string searchString)
        {
        ...
        private IQueryable<Project> GetUserProjectParticipationsQuery(LmPlatformModelsContext context, int userId,
            string searchString)
        {
        ...
        private IQueryable<Project> GetUserProjectsSortedQuery(IQueryable<Project> query, string sortingPropertyName,
            bool desc)
        {
            switch (sortingPropertyName)
            {
                case "UserQuentity":
                    return desc ? query.OrderByDescending(e => e.ProjectUsers.Count) : query.OrderBy(e => e.ProjectUsers.Count);
                case "Title":
                    return desc ? query.OrderByDescending(e => e.Title) : query.OrderBy(e => e.Title);
                case "CreatorName":
                    if (desc)
                        return query.OrderByDescending(e =>
                            e.Creator.Student == null ? e.Creator.Lecturer.LastName : e.Creator.Student.LastName);
                    else
                        return query.OrderBy(e =>
                            e.Creator.Student == null ? e.Creator.Lecturer.LastName : e.Creator.Student.LastName);
                default:
                    return desc ? query.OrderByDescending(e => e.DateOfChange) : query.OrderBy(e => e.DateOfChange);
            }
        }
    }
}